...
void dxdy_g4sbs_v2(int entries_input = -1, TString configfileinput="sbs4_30p_inel"){//main
  ...
  while (std::getline(summaryfile, line)) {
    ...
    std::getline(iss, token, ',');  // Extract 7th param: ibeam
    double ibeam = std::stod(token);
  ...
  for (const auto& job : JobSummaries) {
    std::cout << job.jobid <<", "<< job.Nthrown<<", "<< job.Ntried <<", "<< job.genvol <<", "<< job.luminosity <<", "<< job.ebeam <<", "<< job.ibeam<<std::endl; 
  ...
  P->Branch("mc_sigma", &mc_sigma_out, "mc_sigma/D");
  ...
  TH1D* h_mc_sigma = new TH1D("h_mc_sigma","mc_sigma",100,0,100e-33);
