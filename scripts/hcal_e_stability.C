...
// set location and name for output file 
  ...
  TString outputfilename = "../output/hcal_e_stability_test.root";
 ...
 // Declare outfile
  ...
  cout<<"writing to file: "<< outputfilename <<endl;
   ...
   // Load Histograms
  TH2D *hist_data_orig = (TH2D*)f1->Get("hcal_dx__hcal_e");
  TH2D *hist_proton_orig = (TH2D*)f2->Get("hcal_dx__hcal_e");
  TH2D *hist_neutron_orig = (TH2D*)f3->Get("hcal_dx__hcal_e");
 ...
 std::vector<double> xSlices ={0.017,0.018,0.019,0.020,0.021,0.022,0.023,0.024,0.025,0.026,0.027,0.028,0.029,0.030,0.031,0.032,0.033,0.034,0.035,0.036};
  double hcal_e_max= 1;
  ...
  SliceAndProjectHistogram_xMaxFixed(hist_2D_proton, xSlices, hist_vector_p, hcal_e_max, "hcal_e","dx","p");
  SliceAndProjectHistogram_xMaxFixed(hist_2D_neutron, xSlices, hist_vector_n, hcal_e_max, "hcal_e","dx","n");
  SliceAndProjectHistogram_xMaxFixed(hist_2D_data, xSlices, hist_vector_data, hcal_e_max, "hcal_e","dx","data");
...
//      hist_1d_test_data->Fit(overall_fit,"Q");
	 ...
	 cout<< "ratio: scale_n / scale_p = "<< scale_n <<" / " <<scale_p <<" = "<< Rsf <<" +/- "<< Rsf_err<<endl;
     ...
     for (int sliceid = 0 ; sliceid < xSlices.size() ; sliceid++)
       {
	 hist_residual = GetResidualHistogram(hist_vector_data[sliceid], fit_vector[sliceid]);
     ...
     double x[xSlices.size()];
     double y[xSlices.size()];
     double x_err[xSlices.size()];
     double y_err[xSlices.size()];
     for (int sliceid = 0 ; sliceid < xSlices.size() ; sliceid++)
       {
...
void customizeGraph(TGraphErrors *graph, int markerStyle, int markerColor, double markerSize, 
     ...
     //// Plot the slices and fits 
     if (hist_vector_data.size() != fit_vector.size()) {
       std::cerr << "Error: The number of histograms and fits must be the same." << std::endl;
       return;
     }
 ...
 // Create a canvas to draw the histogram
    ...
    for (size_t i = 0; i < xSlices.size(); ++i) {
        double x = xSlices[i];
 ...
 printParsedTitle(hist_2D_data,"data");
       ...
       // const char* hist_data_title = hist_2D_data->GetTitle();
       // TCanvas* titlecanvas = new TCanvas("titlecanvas", "Cuts", 1000, 600);    
       // // Create a TLatex object to draw the title
       // TLatex latex;
       // latex.SetTextSize(0.05);  // Adjust text size
       // latex.DrawLatex(0.02, 0.9, hist_data_title);  // Draw title at upper left corner
       // // Update the canvas
       // titlecanvas->Update();
       // // Optionally, save the canvas as an image
       // titlecanvas->SaveAs("global_cuts.pdf");



 // //// canvas
 //     TCanvas *graphcanvas0p05 = new TCanvas("graphcanvas0p05","graphcanvas0p05",800,600);  
 //     adjustCanvas(graphcanvas0p05,0.05,0.05,0.05,0.05);
 //     Rsf_graph->Draw("AP");
 //     graphcanvas0p05->Update();
 ...
 //     hist_result_vector_p[4]->Draw("same");
 //     hist_result_vector_n[4]->Draw("same");
 //     poly_fit_result[4]->Draw("same");
 //     c2->Update();

 //     TCanvas *c3= new TCanvas("c3","c3",800,600);
 //     hist_residual_vector[4]->Draw();
 //     c3->Update();
