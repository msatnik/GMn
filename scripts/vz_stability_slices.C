...
void vz_stability_slices(){ // main
  ...
  TString outputfilename = "../output/vz_stability_slices_test.root";
 ...
 // Set up the slices for the Y projections. 
  ...
  std::vector<double> xSlices = {0.040,0.045,0.050, 0.055, 0.060 ,0.065, 0.070, 0.075, 0.080};
  //std::vector<double> xSlices = {-0.080, -0.075, -0.070, -0.065, -0.060, -0.055, -0.050};
 ...
 // use the function to make the slices and projections
  SliceAndProjectHistogram_slices(hist_2D_proton, xSlices, hist_vector_p, "vz","dx","p");
  SliceAndProjectHistogram_slices(hist_2D_neutron, xSlices, hist_vector_n, "vz","dx","n");
  SliceAndProjectHistogram_slices(hist_2D_data, xSlices, hist_vector_data, "vz","dx","data");
  ...
  for (int sliceid = 0 ; sliceid < nSlices ; sliceid++)
    {
      ...
      Fit->SetParLimits(2, -0.1,0.1); // shift_p less than +- 10cm
      Fit->SetParLimits(3, -0.1,0.1); // shift_n less than +- 10cm
      Fit->SetNpx(500);
