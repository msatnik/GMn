...
void dxdy_g4sbs_fid(int entries_input = -1, int kine = 4 ){//main
  ...
  string configfilename = "/w/halla-scshelf2102/sbs/msatnik/GMn/config/sbs4_sim_sf03.cfg";
  ...
  //TFile *fout = new TFile(outputfilename,"RECREATE");
   TFile *fout = new TFile("../output/sbs4_sim_sf03_fid.root", "RECREATE" );
  ...
  while( currentline.ReadLine( configfile ) && !currentline.BeginsWith("#") ){
	...
	TString sval = ( (TObjString*)(*tokens)[1] )->GetString();
        dx_pn = sval.Atof();
	cout << "Loading proton-neutron seperation dx_pn: " << dx_pn << endl;
      }
      if( skey == "N_tries" ){
	TString sval = ( (TObjString*)(*tokens)[1] )->GetString();
        N_tries = sval.Atof();
...
//// define hcal active area
 ...
 cout<<endl << "Opened up TChain with nentries: " << C->GetEntries() << ". After globalcut: " << Nevents << "." << endl << endl;
  ...
  for( Long64_t nevent = 1; nevent <max; nevent++){
    ...
    //double corrected_weight = simc_weight * Luminosity * Generation_Volume / N_tries; // this SHOULD work but isn't
    //double corrected_weight = simc_weight;

    double corrected_weight = mc_sigma;
    ...
    if ( hcalaaON( xexpect_HCAL , yexpect_HCAL , hcalaa))// was HCALx, HCALy should this be actual or expected? 
      {
