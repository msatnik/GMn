...
void dy_fid_stability(){ // main
  ...
  TString outputfilename = "../output/dy_fid_stability_test.root";
  ...
  // Load Histograms
  TH2D *hist_data_orig = (TH2D*)f1->Get("hcal_dx__nsigy_fid");
  TH2D *hist_proton_orig = (TH2D*)f2->Get("hcal_dx__nsigy_fid");
  TH2D *hist_neutron_orig = (TH2D*)f3->Get("hcal_dx__nsigy_fid");
  ...
  std::vector<double> xSlices = {0,0.2, 0.4,0.6,0.8,1,1.2,1.4,1.6,1.8,2,2.2,2.4};
  double max= 3;
  ...
  SliceAndProjectHistogram_xMaxFixed(hist_2D_proton, xSlices, hist_vector_p, max, "nsigy_fid","dx","p");
  SliceAndProjectHistogram_xMaxFixed(hist_2D_neutron, xSlices, hist_vector_n, max, "nsigy_fid","dx","n");
  SliceAndProjectHistogram_xMaxFixed(hist_2D_data, xSlices, hist_vector_data, max, "nsigy_fid","dx","data");
  ...
  customizeGraph(Rsf_graph, 33, kBlue, 3,"","nsigma fiducial y","Rsf");
  void customizeGraph(TGraphErrors *graph, int markerStyle, int markerColor, double markerSize, 
		      const char* graphTitle, const char* xAxisLabel, const char* yAxisLabel);
