...
void dxdy_g4sbs(int entries_input = -1, int kine = 4 ){//main
  ...
  //// set location to config file
   ...
   string configfilename = "/w/halla-scshelf2102/sbs/msatnik/GMn/config/sbs4_sim_30p.cfg";

   // Declare outfile
   // TFile *fout = new TFile( Form("../output/sbs%d.root",kine), "RECREATE" );
  TFile *fout = new TFile("../output/sbs4_sbs30p_sf0p359.root", "RECREATE" );
 ...
 cout<<endl << "Opened up TChain with nentries: " << C->GetEntries() << ". After globalcut: " << Nevents << "." << endl << endl;
  ...
  for( Long64_t nevent = 1; nevent <max; nevent++){
    ...
    h_dx_HCAL ->Fill(dx, mc_sigma);
    h_dy_HCAL ->Fill(dy, mc_sigma);
    h_dxdy_HCAL ->Fill(dy,dx);
    h_W2 ->Fill(ekineW2);

    if (mc_nucl == 1) // equals 1 for proton, (0 for neutron?)
      {
	h_dx_HCAL_p ->Fill(dx, mc_sigma);
      }
    if (mc_nucl == 0)
      {
	h_dx_HCAL_n ->Fill(dx, mc_sigma);
      }
