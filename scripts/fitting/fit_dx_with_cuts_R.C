...
// MAIN
void fit_dx_with_cuts_R(TString KineString="sbs4_30p", int PolyOrder_input = 2){//main
  ...
  if (PolyOrder_input == -1){
    cout<<"PolyOrder_input == -1. Fitting with no background"<<endl;
  }
  
  if((PolyOrder_input < -1) || PolyOrder_input > 6){
  ...
  TString outputfilelocation;
  TString outputfilename;
  
  if(PolyOrder == -1){
    // set location and name for output file 
    outputfilelocation="../../output/final_Rsf_fits/"+KineString+"/noBackground";
    outputfilename = outputfilelocation +"/"+ KineString+"_noBackground.root";
  }else{
    // set location and name for output file 
    outputfilelocation="../../output/final_Rsf_fits/"+KineString+"/poly"+PolyOrder;
    outputfilename = outputfilelocation +"/"+ KineString+"_poly"+PolyOrder+".root";
  }
  ...
  hist2->SetLineColorAlpha(kViolet,0.9);
  hist2->SetLineWidth(1);
  hist2  ->SetFillColorAlpha(kViolet,0.1);
  hist2 ->Draw("hist E");
  ...
  mc_unscaled_leg->AddEntry(hist2,"Proton simc (unscaled unshifted)","f l");
