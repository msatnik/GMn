...
void fit_dx_with_cuts_2_R(TString KineString="sbs4_30p", int PolyOrder_input = 2){//main
  ...
  if(PolyOrder ==-1){
    outputfilelocation="../../output/final_Rsf_fits_twice_fit/"+KineString+"/noBackground";
    outputfilename = outputfilelocation +"/"+ KineString+"_noBackground.root";
  }else{
    // set location and name for output file 
    outputfilelocation="../../output/final_Rsf_fits_twice_fit/"+KineString+"/poly"+PolyOrder;
