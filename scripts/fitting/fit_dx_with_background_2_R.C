...
// MAIN
void fit_dx_with_background_2_R(TString KineString="sbs4_30p", TString bg_string="antidy", int PolyOrder_input=5){//main
  ...
  TString outputfilelocation="../../output/final_Rsf_fits_twice_fit/"+KineString+"/" +bg_string;
  ...
  TH1D *hist_n =  (TH1D*)hist3->Clone("hist");
  ...
  TH1D *hist_n_2 =  (TH1D*)hist3->Clone("hist_2");
  

  if (!hist_data_2 || !hist_p_2 || !hist_n_2 || !histBG) {
  ...
  TF1 *FitBG = new TF1("FitBG",Form("pol%i",polyorder),xminBG,xmaxBG); // make sure this matches polyorder
  FitBG->SetNpx(500);
  ...
  fitHandler2.setPolynomialCoefficients(polyresultBG);
 

  fitHandler2.fitDataWithBG(hist_data_dont_draw_2); // bug in here that corrupts it and crashes if you try and draw

  TF1 *fit_result2 = (TF1*)fitHandler2.fitFunc->Clone("fit_result_2");// bug in here that corrupts it and crashes if you try and draw
  ...
  TH1D *hist_n_result_2 = utilityHandler.ScaleAndShiftHistogram(hist_n_2, fitHandler2.scale_n,fitHandler2.shift_n);
  ...
  for (int i = 0 ; i < polyorder+1; i++)
    {
      double scaled_param = fitHandler.BGscale*polyresultBG[i];
