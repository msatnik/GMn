...
void fillHistogramWithGaussian(TH1D* histogram, double mean, double sigma, int numEntries);
void addPolynomialBackground(TH1D* histogram, double a, double h, double k);
...
// MAIN
void fit_dx_debug(){ // going to instead make histograms out of gaussians to try and debug the fitting. 
  ...
  // //TFile *f2 = TFile::Open("/w/halla-scshelf2102/sbs/msatnik/GMn/output/sbs4_50p_simc_deep.root"); // Load rootfile
  // TFile *f3 = TFile::Open("/w/halla-scshelf2102/sbs/msatnik/GMn/output/sbs4_simc_30p_deen.root"); 
  // // TFile *f3 = TFile::Open("/w/halla-scshelf2102/sbs/msatnik/GMn/output/sbs4_50p_simc_deen.root"); 


  // // Load Histograms
  // TH1D *hist1 = (TH1D*)f1->Get("h_dx_HCAL");
  // TH1D *hist2 = (TH1D*)f2->Get("h_dx_HCAL");
  // TH1D *hist3 = (TH1D*)f3->Get("h_dx_HCAL");

// Create a histogram
    histP = new TH1D("histP", "Gaussian Histogram", 500, -4, 3);
    histN = new TH1D("histN", "Gaussian Histogram", 500, -4, 3);

    double p_mean_test = -0.5;
    double p_sigma_test = 0.19;
    double p_nentries_test = 100000;
    double n_mean_test = 0.03;
    double n_sigma_test = 0.18;
    double n_nentries_test = 80000;

    // histP and histN are going to act as my simc input. They are shifted to the right slightly. 
    fillHistogramWithGaussian(histP, p_mean_test, p_sigma_test, p_nentries_test);
    fillHistogramWithGaussian(histN, n_mean_test, n_sigma_test, n_nentries_test);

    // building a dx histogram that is two more gaussians plus a backgournd, with a shift more to the left to act as my data. 
    double p_mean_test_data = -0.65;
    double p_sigma_test_data = 0.19;
    double p_nentries_test_data = 10000;
    double n_mean_test_data = 0.0;
    double n_sigma_test_data = 0.18;
    double n_nentries_test_data = 7000;

    TH1D *hist_dx_HCAL =  new TH1D("hist_dx_HCAL","hist_dx_HCAL",500,-4,3);
    histP_data = new TH1D("histP_data", "Gaussian Histogram", 500, -4, 3);
    histN_data = new TH1D("histN_data", "Gaussian Histogram", 500, -4, 3);
    // fill the gausians with the 
    fillHistogramWithGaussian(histP_data, p_mean_test_data, p_sigma_test_data, p_nentries_test_data);
    fillHistogramWithGaussian(histN_data, n_mean_test_data, n_sigma_test_data, n_nentries_test_data);
     ...
     addPolynomialBackground(background_hist, a, h, k);
     hist_dx_HCAL ->Add(histP_data, histN_data);
     hist_dx_HCAL ->Add(hist_dx_HCAL,background_hist);

     
 

     TH1D *hist1 = (TH1D*)hist_dx_HCAL->Clone("hist1");
  ...
  double initialParameters[9] = {1,1,0,0,1,1,1,1,1};
   ...
   // Create custom fit function
    ...
    upperPad->cd();
     hist1->Draw("E");
     fitFunc->Draw("sames");
    poly_result ->SetLineColor(kBlue); 
    poly_result ->SetFillColorAlpha(kBlue,0.35);
    poly_result ->SetFillStyle(1001);
    poly_result ->Draw("sames");
    histP_clone->SetLineColorAlpha(kGreen,0.8);
    histP_clone ->SetLineWidth(2);
    histP_clone ->SetFillColorAlpha(kGreen,0.35);
    histP_clone ->Draw("hist sames E");
    histN_clone ->SetLineColorAlpha(kMagenta,0.8);
    histN_clone ->SetLineWidth(2);
    histN_clone ->SetFillColorAlpha(kMagenta,0.35);
    histN_clone ->Draw("E sames hist");
 ...
 legend->AddEntry("", Form("   shifted by %.4f ", Nshift_result), "");
  ...
  legend->AddEntry("", Form("R= %.4f ", Ratio), "");
  legend->Draw();
  ...
  //residual_hist ->Draw("E");
   ...
   histP->Draw("hist E");
   histN->Draw("same hist E");

   TCanvas *ctest = new TCanvas("ctest","ctest",800,600);
   histP_data ->Draw("hist");
   histN_data ->Draw("same hist");
...
// function to add a given 2nd order polynomial background to a histogram 
void addPolynomialBackground(TH1D* histogram, double a, double h, double k) {
    int numBins = histogram->GetNbinsX();
    double binWidth = histogram->GetBinWidth(1); // Assume uniform bin width

    // Iterate over histogram bins and add polynomial background
    for (int i = 1; i <= numBins; ++i) {
        double binCenter = histogram->GetBinCenter(i);
        double background = a*pow((binCenter-h),2)+k;
        double binContent = histogram->GetBinContent(i);
        histogram->SetBinContent(i, binContent + background);
    }
}



// Function to fill a histogram with a Gaussian distribution
void fillHistogramWithGaussian(TH1D* histogram, double mean, double sigma, int numEntries) {
    TRandom3 random; // Random number generator

    // Fill the histogram with Gaussian distribution
    for (int i = 0; i < numEntries; ++i) {
        double value = random.Gaus(mean, sigma); // Generate random number from Gaussian distribution
