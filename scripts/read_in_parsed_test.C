...
void read_in_parsed_test(TString configfileinput="sbs4_30p_test"){ // main
...
// need to check binning on all of these histos. Just quickly testing now. 
  ...
  std::string ps_e_study_string = TrackQualityCutString + "&&"+TargetVertexCutString +"&&" +W2CutString+ "&&"+FidXCutString +"&&"+ FidYCutString + "&&"+e_over_p_CutString + "&&"+ HCal_Energy_CutString +"&&"+ HCal_Shower_atime_CutString ;
//// Draw the 2D histogram
  C->Draw("hcal_dx:bb_ps_e>>hcal_dx__ps_e(100, 0, 2, 200, -4, 4)", ps_e_study_string.c_str(), "COLZ");
