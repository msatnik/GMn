...
void vz_stability(){ // main
  ...
  TString outputfilename = "../output/vz_stability_test.root";
  ...
  // Load Histograms
  TH2D *hist_data_orig = (TH2D*)f1->Get("hcal_dx__tr_vz");
  TH2D *hist_proton_orig = (TH2D*)f2->Get("hcal_dx__tr_vz");
  TH2D *hist_neutron_orig = (TH2D*)f3->Get("hcal_dx__tr_vz");
 ...
 // Set up the slices for the Y projections. 
  std::vector<double> xSlices = {0.05, 0.052, 0.054, 0.056, 0.058, 0.06, 0.062, 0.064, 0.066,0.068, 0.07, 0.072, 0.074, 0.076, 0.078, 0.08};
  double mean = 0;
 ...
 // use the function to make the slices and projections
  SliceAndProjectHistogram_AroundMean(hist_2D_proton, xSlices, hist_vector_p, mean, "vz","dx","p");
  SliceAndProjectHistogram_AroundMean(hist_2D_neutron, xSlices, hist_vector_n, mean, "vz","dx","n");
  SliceAndProjectHistogram_AroundMean(hist_2D_data, xSlices, hist_vector_data, mean, "vz","dx","data");
  ...
  customizeGraph(Rsf_graph, 33, kBlue, 3,"","vz","Rsf");
  void customizeGraph(TGraphErrors *graph, int markerStyle, int markerColor, double markerSize, 
		      const char* graphTitle, const char* xAxisLabel, const char* yAxisLabel);
  ...
  for (size_t i = 0; i < xSlices.size(); ++i) {
    ...
    TLine* line_neg = new TLine(-x, hist_2D_data->GetYaxis()->GetXmin(), -x, hist_2D_data->GetYaxis()->GetXmax());
    line->SetLineColor(kRed);  // Set line color 
    line->SetLineWidth(2);     // Set line width
    line->Draw("SAME");
    line_neg->SetLineColor(kRed);  // Set line color 
    line_neg->SetLineWidth(2);     // Set line width
    line_neg->Draw("SAME");
  }
  ...
  for (size_t i = 0; i < xSlices.size(); ++i) {
    ...
    TLine* line_neg = new TLine(-x, hist1D_data->GetMinimum(), -x, hist1D_data->GetMaximum());
    line->SetLineColor(kRed);  // Set line color (e.g., red)
    line->SetLineWidth(2);     // Set line width
    line->Draw("SAME");
    line_neg->SetLineColor(kRed);  // Set line color (e.g., red)
    line_neg->SetLineWidth(2);     // Set line width
    line_neg->Draw("SAME");
  }
  ...
  for (size_t i = 0; i < xSlices.size(); ++i) {
    ...
    TLine* line_neg = new TLine(-x, hist1D_p->GetMinimum(), -x, hist1D_p->GetMaximum());
    line->SetLineColor(kRed);  // Set line color (e.g., red)
    line->SetLineWidth(2);     // Set line width
    line->Draw("SAME");
    line_neg->SetLineColor(kRed);  // Set line color (e.g., red)
  ...
  for (size_t i = 0; i < xSlices.size(); ++i) {
    ...
    TLine* line_neg = new TLine(-x, hist1D_n->GetMinimum(), -x, hist1D_n->GetMaximum());
    line->SetLineColor(kRed);  // Set line color (e.g., red)
    line->SetLineWidth(2);     // Set line width
    line->Draw("SAME");
    line_neg->SetLineColor(kRed);  // Set line color (e.g., red)
