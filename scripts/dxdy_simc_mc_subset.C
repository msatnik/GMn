...
void dxdy_simc_mc_subset(int entries_input = -1, TString configfileinput="sbs4_30p_simc", int FirstJob=0, int LastJob=65){//main
  
  gStyle->SetNumberContours(255);

  if ( !(FirstJob >= 0 && FirstJob<=LastJob) )
    {
      cout<<"error with job number input"<<endl;
      return; 
    }

  std::string summaryfilePathInput;
  std::string path;
  
  if (configfileinput == "sbs4_30p_simc_deen"){ 
    summaryfilePathInput ="/volatile/halla/sbs/seeds/simc/p390sf_sbs4_sbs30p_simc/simcout/p390sf_sbs4_sbs30p_simc_deen_summary.csv"; 
    path ="/volatile/halla/sbs/seeds/simc/p390sf_sbs4_sbs30p_simc/replayed_p390sf_sbs4_sbs30p_simc_deen_job_";
  } else if (configfileinput == "sbs4_30p_simc_deep"){
    summaryfilePathInput ="/volatile/halla/sbs/seeds/simc/p390sf_sbs4_sbs30p_simc/simcout/p390sf_sbs4_sbs30p_simc_deep_summary.csv"; 
    path ="/volatile/halla/sbs/seeds/simc/p390sf_sbs4_sbs30p_simc/replayed_p390sf_sbs4_sbs30p_simc_deep_job_";
  }else if (configfileinput == "sbs8_70p_simc_deep"){
    summaryfilePathInput ="/volatile/halla/sbs/seeds/simc_clean/p63797sf_sbs8_sbs70p_simc/simcout/p63797sf_sbs8_sbs70p_simc_deep_summary.csv";
    path ="/volatile/halla/sbs/seeds/simc_clean/p63797sf_sbs8_sbs70p_simc/replayed_p63797sf_sbs8_sbs70p_simc_deep_job_";
  }
  else if(configfileinput == "sbs8_70p_simc_deen"){
    summaryfilePathInput = "/volatile/halla/sbs/seeds/simc_clean/p63797sf_sbs8_sbs70p_simc/simcout/p63797sf_sbs8_sbs70p_simc_deen_summary.csv";
    path ="/volatile/halla/sbs/seeds/simc_clean/p63797sf_sbs8_sbs70p_simc/replayed_p63797sf_sbs8_sbs70p_simc_deen_job_";
  }else if (configfileinput == "sbs9_70p_simc_deep"){
    summaryfilePathInput ="/volatile/halla/sbs/ewertz/simc/p657sf_sbs9_sbs70p_simc_g4sbs_reg_digi/simcout/p657sf_sbs9_sbs70p_simc_deep_summary.csv";
    path ="/volatile/halla/sbs/ewertz/simc/p657sf_sbs9_sbs70p_simc_g4sbs_reg_digi/replayed_p657sf_sbs9_sbs70p_simc_deep_job_";
  }
  else if(configfileinput == "sbs9_70p_simc_deen"){
    summaryfilePathInput ="/volatile/halla/sbs/ewertz/simc/p657sf_sbs9_sbs70p_simc_g4sbs_reg_digi/simcout/p657sf_sbs9_sbs70p_simc_deen_summary.csv";
      path ="/volatile/halla/sbs/ewertz/simc/p657sf_sbs9_sbs70p_simc_g4sbs_reg_digi/replayed_p657sf_sbs9_sbs70p_simc_deen_job_";
  }else{
    cout<<"error with the configuration name"<<endl;
    return; 
  }
  ...
  // Add files in the specified range to the TChain
  for (int i = FirstJob; i <= LastJob; ++i) {
    std::string filename = path + std::to_string(i) + ".root";
  ...
  std::string outputfilename = std::string("../output/") + configfileinput.Data() +"_subset_"+std::to_string(FirstJob) +"_"+std::to_string(LastJob)+ ".root";
  ...
  cout<<"Running jobs "<<FirstJob<<" through "<<LastJob<<endl;
  
  // loop over and get the sum of Ntried
  double Ntried_sum = 0;
  for (int i = FirstJob ; i<=LastJob; i++) {
    Ntried_sum += JobSummaries[i].Ntried;
  }
  ...
  for( Long64_t nevent = 1; nevent <max; nevent++){
    ...
    // double ebeam_c = E_e - ( (BBtr_vz[0]+l_tgt/2.0) * lh2tarrho * lh2dEdx + lh2uwallthick * rho_al * aldEdx );//correct for energy loss of beam electron as it passes into the target material towards the vertex position. (should this be the d2 vars?)

    double ebeam_c = E_e - ( (BBtr_vz[0]+l_tgt/2.0) * ld2tarrho * ld2dEdx + ld2uwallthick * rho_al * aldEdx );//correct for energy loss of beam electron as it passes into the target material towards the vertex position. (trying with the d2 vars)
    ...
    ebeam_o =  ebeam_c - celldiameter/2.0/sin(etheta) * ld2tarrho * ld2dEdx; //Second energy correction accounting for energy loss leaving target. (Although S doesn't seem to use this) (It doesn't look like I'm using this either)
